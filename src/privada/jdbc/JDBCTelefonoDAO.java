...
public class JDBCTelefonoDAO extends JDBCGenericDAO<Telefono, Integer> implements TelefonoDAO{
	...
	public void create(Telefono telefono) {
		// TODO Auto-generated method stub
		conexionUno.update("INSERT Telefono VALUES (" + telefono.getCodigo()+",'"+ telefono.getNumero()+"','"+telefono.getTipo()+"','"+telefono.getOperadora()+"','"+ telefono.getUsuario()+"')");
	...
	public Telefono read(Integer id) {
		// TODO Auto-generated method stub
			...
			try {
				if(rs != null && rs.next()) {
					telefono= new Telefono(rs.getInt("tel_codigo"),rs.getString("tel_numero"),rs.getString("tel_tipo"),rs.getString("tel_operadora"));
				}
	...
	public List<Telefono> buscarPorIdTelfono(String telf) {
		// TODO Auto-generated method stub
				...
				try {
					while(rs.next()) {
						telefono = new Telefono(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4));
						list.add(telefono);
						//list.add(new Telefono(rs.getInt("tel_codigo"),rs.getString("tel_numero"),rs.getString("tel_tipo"),rs.getString("tel_operadora")));
